...
summary(stormdataclean)
```
#### Clean up the event type field. There should only be 48 event types. We'll use the stringdist package.
```{r}
stormdataclean$EVTYPE <- toupper(stormdataclean$EVTYPE)
stormdataclean$EVTYPE <- trimws(stormdataclean$EVTYPE)
```
...
storm.data.by.type <- summarize(storm.data.by.type, TOTALCROPDMG = sum(TOTALCROPDMG), TOTALPROPDMG = sum(TOTALPROPDMG))
```
